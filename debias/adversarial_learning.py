...
def race_scores(sess, pred, data_p, client, race_direction, gender_direction):
    ...
    df["race_score"] = df["word"].map(
        lambda w: (sess.run(pred, feed_dict={data_p: np.array([[client.wv.get_vector(w),client.wv.get_vector(w),client.wv.get_vector(w)]])}).dot(race_direction)))
    df.sort_values(by="race_score", inplace=True)

    df["gender_score"] = df["word"].map(
        lambda w: (sess.run(pred, feed_dict={data_p: np.array([[client.wv.get_vector(w),client.wv.get_vector(w),client.wv.get_vector(w)]])}).dot(gender_direction)))
    df.sort_values(by="race_score", inplace=True)
...
def main(client, analogies, indices, embed, gender_direction, race_direction):
  # Edit the training parameters below to experiment with different training runs.
  # For example, try
  ...
  bw = client.wv.get_vector("black_woman")
  bw = np.array([[bw,bw,bw]])
  print("=====")
  print(sess.run(pred, feed_dict={data_p:bw}))
  ...
  print_knn(client, sess.run(pred, feed_dict={data_p: in_arr}),
            NUM_ANALOGIES)
  race_scores(sess, pred, data_p, client, race_direction, gender_direction)
