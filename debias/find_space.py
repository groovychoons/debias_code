...
def load_vectors(client, analogies):
    ...
    for word in words_unfiltered:
        try:
            vecs.append(_np_normalize(client.get_vector(word)))
...
def find_gender_direction(embed, indices, client):
    ...
    m = []
    for wf, wm in pairs:
        m.append((_np_normalize(client.get_vector(wf))) - (_np_normalize(client.get_vector(wm))))
    m = np.array(m)
...
def race_scores(client, gender_direction, analogies):
    ...
    words = set()
    words2 = list(client.key_to_index.keys())
    ...
    df["race_score"] = df["word"].map(
        lambda w: client.get_vector(w).dot(gender_direction))
...
def main(client, analogies):
    ...
    print("race direction: %s" % str(gender_direction.flatten()))
    ...
    word_vec = client.get_vector(WORD)
